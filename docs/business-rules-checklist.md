...
| 검수 완료(정상 판정) 시 대상 재고를 정상 재고로 복귀시키고, 불량 판정 시 폐기 등록(`DisposalReason.RETURN_DEFECT`)으로 연계 | 미구현 — Inbound의 불량 상품 이동이 별도 도메인 부재로 범위 밖이었던 것과 동일하게, Inventory/Disposal 리포지토리 접근이 필요해 후속 PR로 분리(`ReturnRequestService` 상단 주석 참고) | 없음 | ❌ 미구현 |
