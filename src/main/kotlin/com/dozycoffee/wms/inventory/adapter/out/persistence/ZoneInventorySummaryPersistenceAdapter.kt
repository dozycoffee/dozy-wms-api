...
class ZoneInventorySummaryPersistenceAdapter(
     ...
     * 합치는 이유.
     */
    override fun findAll(): Flow<ZoneInventorySummaryResult> = flow {
        val capacities = fetchZoneCapacities()
        val quantities = fetchQuantitiesByZone()
    ...
    private suspend fun fetchZoneCapacities(): List<ZoneCapacityRow> {
        return databaseClient.sql(ZONE_CAPACITY_QUERY)
            .map { row, _ ->
                ZoneCapacityRow(
    ...
    private suspend fun fetchQuantitiesByZone(): Map<Long, Map<QualityStatus, Int>> {
        val rows = databaseClient.sql(ZONE_QUANTITY_QUERY)
            .map { row, _ ->
                ZoneQuantityRow(
                    zoneId = requireNotNull(row.get("zone_id", java.lang.Long::class.java)).toLong(),
    ...
    companion object {
        ...
        private const val ZONE_QUANTITY_QUERY = """
